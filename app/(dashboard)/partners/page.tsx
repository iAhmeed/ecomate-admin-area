...
export default function PartnersPage() {
  ...
  const fetchPartners = async () => {
    try {
      ...
      if (data.status === "SUCCESS") setPartners(data.brands || []);
    } catch (err) {
  ...
  const openAdd = () => {
    setEditing(null); setName(""); setImageFile(null); setImagePreview(""); setError(""); setModalOpen(true);
  };

  const openEdit = (partner: Partner) => {
    setEditing(partner); setName(partner.name); setImageFile(null); setImagePreview(partner.image?.url || ""); setError(""); setModalOpen(true);
  };

  const uploadImage = async (file: File): Promise<PartnerImage | null> => {
    const formData = new FormData();
    formData.append("file", file);
    const res = await fetch("/api/upload", { method: "POST", body: formData });
    const data = await res.json();
    if (data.status === "SUCCESS") return { url: data.result.secure_url, publicId: data.result.public_id };
    return null;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!editing && !imageFile) { setError("Please upload a partner logo"); return; }
    setSubmitting(true);
    try {
      ...
      const url = editing ? `/api/partners/${editing._id}` : "/api/partners";
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 animate-fade-up">
        <div>
          <h1 className="font-heading text-4xl font-bold text-white mb-1">Trusted Partners</h1>
          <p className="text-xl" style={{ color: 'rgba(255,255,255,0.4)' }}>Manage your agency&apos;s trusted partners and brands.</p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="rounded-2xl p-8 h-40" style={{ background: 'rgba(255,255,255,0.028)', border: '1px solid rgba(255,255,255,0.06)', animation: 'pulse 2s infinite' }} />
          ))}
        </div>
      ) : partners.length === 0 ? (
        <div className="text-center py-20 rounded-2xl" style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <svg className="mx-auto mb-4" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>
          <p className="text-xl font-medium" style={{ color: 'rgba(255,255,255,0.3)' }}>No partners yet</p>
          <button onClick={openAdd} className="mt-4 text-xl font-semibold text-primary hover:underline">Add your first partner</button>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {partners.map((partner, i) => (
            <div key={partner._id}
                 className="rounded-2xl p-8 transition-all duration-300 hover:-translate-y-1 group flex flex-col items-center text-center animate-fade-up"
              ...
              <div className="relative mb-3">
                <img src={partner.image?.url} alt={partner.name}
                     ...
                     style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', padding: '8px' }} />
              </div>
              <h3 className="font-heading text-xl font-semibold text-white mb-3 truncate w-full">{partner.name}</h3>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => setDeleteConfirm(partner._id)}
                        className="p-1.5 rounded-lg transition-colors"
                        style={{ color: 'rgba(255,255,255,0.4)' }}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          ...
          <div className="relative rounded-2xl p-8 w-full max-w-sm animate-fade-up" onClick={(e) => e.stopPropagation()}
            ...
            <h3 className="font-heading text-xl font-bold text-white mb-2">Delete Partner?</h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          ...
          <div className="relative rounded-2xl p-8 w-full max-w-lg animate-fade-up overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-heading text-xl font-bold text-white">{editing ? "Edit Partner" : "Add Partner"}</h3>
            ...
            {error && (
              <div className="mb-5 px-6 py-4 rounded-xl text-xl font-medium"
                   style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444' }}>{error}</div>
            )}

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-xl font-semibold uppercase tracking-wider mb-2" style={{ color: 'rgba(255,255,255,0.45)' }}>Partner Name</label>
              ...
              <div>
                <label className="block text-xl font-semibold uppercase tracking-wider mb-2" style={{ color: 'rgba(255,255,255,0.45)' }}>Logo Image</label>
                <input ref={fileRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                {imagePreview ? (
                  <div className="relative inline-block">
                    <img src={imagePreview} alt="Preview" className="w-20 h-20 rounded-xl object-contain p-2"
                         style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
                    <button type="button" onClick={() => { setImageFile(null); setImagePreview(""); if (fileRef.current) fileRef.current.value = ""; }}
                            className="absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center text-white" style={{ background: '#EF4444' }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                  </div>
                ) : (
                  <button type="button" onClick={() => fileRef.current?.click()}
                          className="w-full py-8 rounded-xl text-xl font-medium flex flex-col items-center gap-2 transition-all"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting}
                        className="flex-1 py-4 rounded-xl text-xl font-bold text-white disabled:opacity-50"
                        style={{ background: 'linear-gradient(135deg, #2563EB, #1d4ed8)', boxShadow: '0 0 20px rgba(37,99,235,0.3)' }}>
                  {submitting ? "Saving..." : editing ? "Update" : "Add Partner"}
                </button>
              </div>
            </form>
          </div>
